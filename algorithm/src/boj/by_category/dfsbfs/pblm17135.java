...
public class pblm17135 {
    ...
    static int[] deltaRow = new int[]{0,-1,0};
    static int[] deltaCol = new int[]{-1,0,1};
    ...
    public static int getMax(int archerCol1, int archerCol2, int archerCol3){
        ...
        for (int row = rowNum; row >= 0; row--) {
            ...
            for (int archerCol : new int[]{archerCol1,archerCol2,archerCol3}) {
                // 성공이면 point +1
                visited = new boolean[rowNum+1][colNum];
                int[] targetInfo = shotShortEnemy(row, archerCol);
                // 기록을 하자 문제조건은 동시에 쏴서 죽는거임.
                if (targetInfo != null ) targets.add(targetInfo[0]+","+targetInfo[1]);
            }
            for (String s : targets) {
                String[] parts = s.split(",");
                int targetRow = Integer.parseInt(parts[0]);
                int targetCol = Integer.parseInt(parts[1]);
                copyMap[targetRow][targetCol] = 0;
                maxPoint++;
            }
    ...
    // 궁수가 최소거리에 있는 적을 잡자. 
    public static int[] shotShortEnemy(int archerRow, int archerCol){
        // 궁수의 좌표를 받아서,
        // 궁수보다 위쪽칸만을 탐색을 하자.
        int dist = 0;
